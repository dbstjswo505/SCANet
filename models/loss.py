...
def reconstruction_l2_loss(f_logit, frames_feat_ref, masked_frames):
    mask = masked_frames.expand_as(f_logit)  # [B, T, D]
    squared_error = (f_logit - frames_feat_ref) ** 2
    masked_loss = squared_error * mask
    return masked_loss.sum() / mask.sum()

def rec_loss(words_logit, words_id, words_mask, num_props, f_logit, masked_frames, frames_feat_ref, ref_words_logit=None, **kwargs):
    
    fl2_loss = reconstruction_l2_loss(f_logit, frames_feat_ref, masked_frames)
    ...
    #NOTE: dynamic calibration
    sce = (torch.flatten(kwargs['sce']))
    dc = torch.sigmoid(sce) + 0.25
    min_nll_loss = min_nll_loss * dc
    ...
    if ref_words_logit is not None:
        ...
        final_loss = final_loss / 2 + fl2_loss*0.2
    
    loss_dict = {
        'final_loss': final_loss.item(),
        'nll_loss': min_nll_loss.mean().item(),
        'fl2_loss': fl2_loss.item(),
    }
    if ref_words_logit is not None:
        loss_dict.update({
