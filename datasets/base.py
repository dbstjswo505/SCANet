...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        vid, duration, timestamps, sentence, sce = self.data[index]
        ...
        return {
            ...
            'words_id': words_id,
            'weights': weights,
            'raw': [vid, duration, timestamps, sentence, sce]
        }
...
def build_collate_data(max_num_frames, max_num_words, frame_dim, word_dim):
    def collate_data(samples):
        ...
        difficulty_ann = np.zeros([bsz, 1]).astype(np.float32)
        for i, sample in enumerate(samples):
        ...
        batch.update({
            'net_input': {
                ...
                'sce': torch.from_numpy(np.asarray(sce).astype(np.float32)),
            }
        })
        return batch
